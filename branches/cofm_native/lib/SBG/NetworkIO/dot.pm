    ...
    foreach my $e ($graph->edges) {
        ...
        foreach my $attr ($graph->get_edge_attribute_names($u, $v)) {
            ...
            if ($source eq 'docking') {
                ...
                $color =
                    mapcolors($scores->{avg_seqid}, 0, 100, '#ff0000',
                    '#00ff00');
            }
            ...
            foreach my $key (keys %$scores) {
                $scorelabel
                    .= sprintf("%s\t%f$newline", $key, $scores->{$key});
            }
            $scorelabel
                .= sprintf("%s\t%f$newline", 'weight', $iaction->weight);
            ...
            my $tooltip = join($newline,
                ...
                sprintf("%s\tvia\t%s", $vlabel, $vsubject), $scorelabel,);

            $str .= "\t\"$ulabel\" -- \"$vlabel\" ["
                . join(', ',
                "color=\"$color\"",     "penwidth=$penwidth",
                "tooltip=\"$tooltip\"", "URL=\"$url\"",
                "];\n");
