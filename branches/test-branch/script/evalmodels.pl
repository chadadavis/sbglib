...
use PBS::ARGV @generic_options, qw/redo=i/;

# the 'models' base directory
my %ops = getoptions @generic_options, qw/redo=i/;
...
use SBG::U::Log;
use SBG::U::List
    qw/mean avg wtavg median sum min max flatten argmax argmin between/;
...
sub _domfile {
    ...
    $log->debug($domfile);
    my $write_target = $model->scores->exists('benchrmsd')
    ...
    foreach my $key (@$keys) {
        ...
        $domio->write($target->domains([ $mapping->{$key} ]))
            if $write_target;
    }
}    # _domfile
...
sub _pdbfile {
    ...
    my $pdbio =
        new SBG::DomainIO::pdb(file => ">${pdbfile}", compressed => 0);
    my $fh = $pdbio->fh();
    ...
    if ($combine) {
        ...
        my $modelasdom =
            $write_target ? $model->combine(keys => $keys) : $model->domains;

        # Get target complex as single domain
        my $targetasdom =
            $write_target ? $target->combine(keys => $mapped_keys) : undef;
