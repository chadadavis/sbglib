...
$points = _perm($points);
is($gh->class($points), 'newmodel1', "Non-axis points, rotated");

is($gh->class($points, [qw/c b o/]),
    'newmodel1', "Non-axis points, rotated, w/ labels");
...
# Take it back out
is($gh->class($objpoints),
    'Multi-point', "Model with multi-point objects, unlabelled");

# Test that label applies to each point in an object
is($gh->class($objpoints, $objlabels),
    'Multi-point', "Model with multi-point objects, labelled");
