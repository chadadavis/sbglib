...
ok($dom->file && -f $dom->file,
    'Finding file for assembly+model:' . $dom->file);
...
my $multidom = SBG::Domain->new(
    pdbid      => '2nn6',
    descriptor => 'CHAIN A B 1 _ to B 100 _ CHAIN C'
);
is($multidom->onechain, undef, '! onechain()');

# Equality
my $equiv =
    SBG::Domain->new(pdbid => '2nn6', descriptor => 'A 10 _ to A 233 _');
