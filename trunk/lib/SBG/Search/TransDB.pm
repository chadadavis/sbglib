...
sub search {
    ...
    my @contacts =
        map { SBG::DB::contact::query(@$_) } values %allentitypairs;
    ...
    my @interactions =
        map { _contact2interaction($_, \%entity2hit) } @repcontacts;
...
sub _distmat {
    ...
    for (my $i = 0; $i < @$contacts; $i++) {
        ...
        for (my $j = $i + 1; $j < @$contacts; $j++) {
            ...
            my $irmsd =
                SBG::DB::irmsd::query($contacts->[$i], $contacts->[$j]);
    ...
    $log->debug("$similar (of ",
        scalar(@$contacts), ") contacts have $sum iRMSDs < Inf");
