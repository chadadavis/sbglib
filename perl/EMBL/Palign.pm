...
@EXPORT = qw(read_align get_msf write_msf get_clustal write_clustal get_block write_block get_afasta write_afasta 
	    get_apir write_apir get_pfam get_hmmalign merge_align do_clustalw_align extract_align get_phylip write_phylip write_gde
...
#
   ...
   for($i=0; $i<=$#data; ++$i) {
	...
	elsif(($_ =~ /^CLUSTAL/) || ($_ =~ /^MUSCLE/)) { $votes{"c"}+=10; last; }
...
# Currently this just ignores the "#" lines and the other junk at the start
   ...
   for($n=0; $n<=$#data; ++$n) {
      ...
      @f = split(/[ \t]+/,$data[$n]);
      chop;
      if((!/^#/) && (!/HMM /) && (!/HMMER /) && (!/- - - -/) && (!/Sequence/) && (!/Copyright/) && (/^[^ ]/) && (length($_)>1) && (!/\/\//)) {
        ($label,$seq) = (split(/ +/));
	$seq =~ s/[\.\-]/ /g;
#	print "Calling $_ sequence data.  Length so far is ",$align->{alen},"\n";
        if(defined($align->{ids}{$label})) {
...
#	    print "Read equiv for ",$stamp_char,$stamp_cut,$stamp_win,"\n";
	    if(defined(@equiv)) { # give up if no STAMP (or wrong field) found
               for($i=0; $i<$align->{alen}; ++$i) {
                   if($equiv[$i] ==1) {
